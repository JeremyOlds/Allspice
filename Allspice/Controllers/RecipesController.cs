...
public class RecipesController : ControllerBase
{
    ...
    public async Task<ActionResult<string>> RemoveRecipe(int recipeId)
    {
        ...
        {
            Account userInfo = await _auth0Provider.GetUserInfoAsync<Account>(HttpContext);
            _recipeService.RemoveRecipe(recipeId, userInfo);
    ...
    public async Task<ActionResult<Recipe>> UpdateRecipe(int recipeId, [FromBody] Recipe recipeData)
    {
        ...
        {
            Account userInfo = await _auth0Provider.GetUserInfoAsync<Account>(HttpContext);
            Recipe recipe = _recipeService.UpdateRecipe(recipeId, recipeData, userInfo);
            return recipe;
        }
        catch (Exception e)
        {
    ...
    public ActionResult<List<Ingredient>> GetIngredientByRecipeId(int recipeId)
    {
        try 
        {
        List<Ingredient> ingredients = _ingredientsService.GetIngredientRecipeById(recipeId);
